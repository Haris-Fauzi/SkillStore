...
@section('content')
<div class="min-h-[80vh] bg-slate-50 dark:bg-slate-800/60 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="name@example.com"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 transition-colors duration-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition text-xs font-medium text-slate-800 dark:text-slate-200 placeholder-slate-300">
                    @error('email')
                ...
                <div class="flex items-center justify-between pt-1">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer">
                        <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 rounded border-slate-200 dark:border-slate-700 transition-colors duration-300 text-blue-600 focus:ring-blue-500/20 transition cursor-pointer">
