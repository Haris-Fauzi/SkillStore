<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div>
        ...
        <select name="category_id" 
                class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('category_id') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition bg-white">
    ...
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $project->title ?? '') }}" placeholder="Contoh: Aplikasi E-Commerce Berbasis Laravel"
               class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('title') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition placeholder-slate-300">
        @error('title')
            <p class="text-[10px] text-rose-500 font-bold mt-1">⚠️ {{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="md:col-span-2">
        ...
        <textarea name="description" rows="5" placeholder="Jelaskan fitur utama, teknologi yang digunakan, dan cara instalasi project kamu..."
                  class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('description') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition placeholder-slate-300">{{ old('description', $project->description ?? '') }}</textarea>
        @error('description')
            <p class="text-[10px] text-rose-500 font-bold mt-1">⚠️ {{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <input type="text" name="version" placeholder="1.0.0" value="{{ old('version', $project->version ?? '1.0.0') }}"
               class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('version') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition placeholder-slate-300">
        @error('version')
            <p class="text-[10px] text-rose-500 font-bold mt-1">⚠️ {{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <input type="number" name="year" placeholder="Format: YYYY (Contoh: 2026)" value="{{ old('year', $project->year ?? date('Y')) }}"
               class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('year') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition placeholder-slate-300">
        @error('year')
            <p class="text-[10px] text-rose-500 font-bold mt-1">⚠️ {{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <input type="url" name="github_url" value="{{ old('github_url', $project->github_url ?? '') }}" placeholder="https://github.com/username/repository"
               class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('github_url') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition placeholder-slate-300">
        @error('github_url')
            <p class="text-[10px] text-rose-500 font-bold mt-1">⚠️ {{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <input type="url" name="demo_url" value="{{ old('demo_url', $project->demo_url ?? '') }}" placeholder="https://project-demo.example.com"
               class="w-full px-4 py-2.5 text-xs font-medium text-slate-800 rounded-xl border @error('demo_url') border-rose-400 focus:ring-rose-500/10 @else border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 @enderror focus:ring-4 transition placeholder-slate-300">
        @error('demo_url')
            <p class="text-[10px] text-rose-500 font-bold mt-1">⚠️ {{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="md:col-span-2 border border-slate-100 bg-slate-50/50 p-4 rounded-2xl">
        ...
        @if(isset($project) && $project->thumbnails)
            <div class="mb-3">
                <img src="{{ asset('storage/'.$project->thumbnails) }}" class="w-32 h-20 object-cover rounded-xl border border-slate-200 shadow-sm" alt="Current Thumbnail">
        ...
        <div class="md:col-span-2 border border-slate-100 bg-slate-50/50 p-4 rounded-2xl">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                @foreach($project->screenshots as $screenshot)
                    <div class="rounded-xl border border-slate-200 overflow-hidden bg-white shadow-sm group relative">
    ...
    <div class="md:col-span-2 border border-slate-100 bg-slate-50/50 p-4 rounded-2xl">
        ...
        @if(isset($project) && $project->file_project)
            <div class="mb-2 flex items-center gap-2 p-2 bg-white rounded-xl border border-slate-100 max-w-md">
