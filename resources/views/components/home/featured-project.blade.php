...
<div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mt-16 pb-2">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8" id="Featured">
        
        <div class="lg:col-span-2 bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl 
                    rounded-3xl border border-slate-200/60 dark:border-slate-800/80 
                    shadow-[0_12px_40px_-15px_rgba(0,0,0,0.05)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] 
                    p-6 md:p-7">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                @forelse($featured as $project)
                    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group">
                        <div>
                            ...
                            <div class="p-5">
                                ...
                                <a href="{{ route('projects.show', $project->slug) }}" class="block text-sm font-bold text-slate-800 dark:text-slate-200 mt-2.5 hover:text-blue-600 dark:hover:text-blue-400 transition line-clamp-2">
                                    {{ $project->title }}
                                </a>
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-span-2 text-center py-12 text-sm text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 shadow-sm transition-colors duration-300">Belum ada project unggulan.</div>
                @endforelse
            </div>
        </div>

        <div class="bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl 
                    rounded-3xl border border-slate-200/60 dark:border-slate-800/80 
                    shadow-[0_12px_40px_-15px_rgba(0,0,0,0.05)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] 
                    p-6 self-start">
            ...
            <div class="space-y-4">
                @forelse($latest as $project)
                    <div class="flex items-center gap-3.5 p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800/60 rounded-xl transition group">
                        <div class="w-11 h-11 bg-blue-50 dark:bg-blue-950/40 text-blue-600 rounded-xl flex flex-col items-center justify-center font-bold text-[10px] uppercase p-1 text-center group-hover:bg-blue-600 group-hover:text-white group-hover:shadow-md group-hover:shadow-blue-500/20 transition duration-300">
                            <span>{{ substr($project->category->category_name ?? 'Web', 0, 3) }}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                            <a href="{{ route('projects.show', $project->slug) }}" class="text-xs font-bold text-slate-800 dark:text-slate-200 line-clamp-1 hover:text-blue-600 dark:hover:text-blue-400 transition">{{ $project->title }}</a>
